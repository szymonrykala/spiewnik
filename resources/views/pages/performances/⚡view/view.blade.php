    ...
    <section class="flex flex-col gap-4">
            ...
            <div class="text-xl">
                @if ($editMode)
                    <input
                        type="date"
                        class="input"
                        wire:model="form.performance_date"
                    >
                @else
        ...
        @if ($errors->any())
            <div class="rounded-md bg-red-100 p-4">
                <ul class="list-inside list-disc text-red-800">
    ...
    @else
        <section class="flex flex-col gap-10">
            <ul class="scrollbar-none flex gap-1 overflow-x-auto rounded-md bg-gray-300 p-2 pb-0">
                @foreach ($this->performanceSongs as $song)
                    ...
                    >
                        ...
                        @admin
                            <span
                                wire:show="currentSongId == {{ $song->id }}"
                                class="block fill-red-500"
                            >
                                <svg
                                    ...
                                    width="18px"
                                    wire:click.stop="removeSong('{{ $song->id }}')"
                                >
                                    <path
                ...
                @admin
                    <li class="px-2">
                        <a
                            href="/songs?performanceIdAssignment={{ $performance?->id }}"
                            wire:navigate
                        >
            ...
            @foreach ($this->performanceSongs as $song)
                ...
                >
                    <h2 class="decor-regular text-4xl">
                        <a
                            wire:navigate
                            href="/songs/{{ $song->id }}"
                        >
                    ...
                    @if ($song->photo_url)
                        <div>
                            <x-image
                                src="{{ asset('storage/' . $song->photo_url) }}"
